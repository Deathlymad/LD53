    ...
    private void decorate(Vector3 offset, float xScale, float yScale)
    {
        ...
        foreach(var v in decorators)
        {
            ...
            if (w <= 0.01f && v.prefab != null)
            {
                ...
                obj.transform.position += offset + new Vector3(UnityEngine.Random.Range(-xScale / 2.0f, xScale / 2.0f), Math.Min(xScale, yScale) * scaleFactor, UnityEngine.Random.Range(-yScale / 2.0f, yScale / 2.0f));
